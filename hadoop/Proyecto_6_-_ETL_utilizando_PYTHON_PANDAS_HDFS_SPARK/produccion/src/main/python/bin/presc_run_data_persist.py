...
def data_persist_hive(spark, df, dfName, partitionBy, mode):
    try:
        ...
        logger.error("Error en el metodo - data_persist_hive(). Por favor chequear el Stack Trace. " + str(exp),exc_info=True)
        raise
    else:
...
def data_persist_postgre(spark, df, dfName, url, driver, dbtable, mode, user, password):
    try:
        ...
        logger.error("Error en el metodo - data_persist_postgre(). Por favor chequear el Stack Trace. " + str(exp),exc_info=True)
        raise
    else:
